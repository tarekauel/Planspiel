...
public class TResourcePrice {
	 ...
	 * @param quality Qualität, die zu dem Preis gehört im Interval ]0;100]
	 * @param price Preis, der der Qualität zugeordnet ist ( > 0 )
	 */
	public TResourcePrice(int quality, int price) {
		if(!checkQuality(quality)) {
			throw new IllegalArgumentException("Die angegebene Qualität ist ungültig '" + quality + "'");
		}
		
		if(!checkPrice(price)) {
			throw new IllegalArgumentException("Der angegebene Preis ist ungültig '" + price + "'");
		}
	 ...
	 * @param quality zu prüfende Qualität
